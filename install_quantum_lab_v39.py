"""Transactional, fail-closed installer for the Quantum Lab V3.9 overlay.

The only accepted transition is an exact V3.8 release to an exact V3.9
...
INSTALLER_VERSION = "QUANTUM LAB V3.9 TRANSACTIONAL INSTALLER · V1"
LOCK_NAME = ".quantum-lab-v39-install.lock"
PARENT_FREEZE_PATH = "FREEZE_CONTRACT_V3_8.json"
V39_FREEZE_PATH = "FREEZE_CONTRACT_V3_9.json"
README_PATH = "quantum_research_lab/README.md"
UI_PATH = "quantum_research_lab/ui.py"

PARENT_FREEZE_RAW_SHA256 = (
    "a267509a153ff2bbec671b963823f818278bad3ee74f8c56e8a5687d805fd7d6"
)
PARENT_FREEZE_SEMANTIC_SHA256 = (
    "5b73497b5a06071aa9411296aa0ed6ec5e924b4372066a1b4a9b9ca240cfc810"
)
PARENT_FROZEN_FILE_COUNT = 114
PARENT_IMMUTABLE_FILE_COUNT = 112
PARENT_FROZEN_PATHS_FINGERPRINT = (
    "90df2180f9965d3721b91b215b2ac7da0eddcde38f890d374863b3079f1c2f19"
)
PARENT_README_SHA256 = (
    "ed8ef9d3bf6f229d8af3ca20f580aaeee578aa01a061908e8f0fecf6aa550541"
)
PARENT_UI_SHA256 = (
    "9aa4bbcd558f9d9c7ca82c2a654065c2b67d2ce832bfff34308a798a53126684"
)

EXPECTED_V39_FROZEN_FILE_COUNT = 129
...
V39_ONLY_FILES = frozenset(V39_TRANSITION_FILES) - {README_PATH, UI_PATH}
...
def _source_inventory(
    ...
    errors: list[str] = []
    try:
        freeze_path = _contained(source_root, V39_FREEZE_PATH, must_exist=True)
        freeze = _read_json_strict(freeze_path)
    except Exception as exc:
        return {}, [], [f"Cannot read source V3.9 freeze: {exc}"]
    frozen = freeze.get("frozen_files") or {}
    if not isinstance(frozen, dict):
        return freeze, [], ["V3.9 frozen_files must be an object."]
    if not (
        freeze.get("frozen_file_count") == EXPECTED_V39_FROZEN_FILE_COUNT
        and len(frozen) == EXPECTED_V39_FROZEN_FILE_COUNT
        and _path_fingerprint(frozen) == EXPECTED_V39_FROZEN_PATHS_FINGERPRINT
        and freeze.get("freeze_contract_sha256") == _freeze_semantic(freeze)
    ):
        errors.append("V3.9 source freeze identity, count, or path fingerprint mismatch.")
    policy = freeze.get("successor_policy") or {}
    if not (
        policy.get("accepted_target_states") == ["V3.8", "V3.9"]
        and policy.get("allowed_v38_superseded_files") == [README_PATH, UI_PATH]
        and policy.get("mixed_or_third_state") == "REJECT"
        and _valid_sha256(policy.get("v39_successor_readme_sha256"))
        and _valid_sha256(policy.get("v39_successor_ui_sha256"))
    ):
        errors.append("V3.9 source successor policy is invalid.")
    missing_from_freeze = set(V39_TRANSITION_FILES) - {V39_FREEZE_PATH} - set(frozen)
    if missing_from_freeze:
        errors.append(
            "V3.9 freeze omits transition paths: " + ", ".join(sorted(missing_from_freeze))
        )
    if frozen.get(PARENT_FREEZE_PATH) != PARENT_FREEZE_RAW_SHA256:
        errors.append("V3.9 freeze does not preserve the exact V3.8 freeze file.")

    rows: list[dict[str, Any]] = []
    for relative in V39_TRANSITION_FILES:
        try:
            path = _contained(source_root, relative, must_exist=True)
            if relative == V39_FREEZE_PATH:
...
def _successor_state(
    ...
    successor = (
        policy.get("v39_successor_readme_sha256"),
        policy.get("v39_successor_ui_sha256"),
    )
...
def authenticate_target(source_root: Path, target_root: Path) -> dict[str, Any]:
    ...
    try:
        source_freeze = _read_json_strict(
            _contained(source_root, V39_FREEZE_PATH, must_exist=True)
        )
        if source_freeze.get("freeze_contract_sha256") != _freeze_semantic(source_freeze):
            raise ValueError("V3.9 source freeze self-hash mismatch")
    except Exception as exc:
        source_freeze = {}
    ...
    try:
        ...
        errors.append(f"Cannot authenticate target V3.8 freeze: {exc}")
    ...
    if not parent_identity:
        errors.append("Target V3.8 freeze raw/semantic identity or inventory mismatch.")
    ...
    if isinstance(parent_files, dict):
        for relative in sorted(set(parent_files) - {README_PATH, UI_PATH}):
            ...
            if not immutable[relative]:
                errors.append(f"Target immutable V3.8 mismatch: {relative}")
    ...
    if state == "V3.8":
        partial_paths = sorted(
            relative
            for relative in V39_ONLY_FILES
            if (target_root / relative).exists() or (target_root / relative).is_symlink()
        )
        if partial_paths:
            errors.append("Partial V3.9 destinations exist on a V3.8 successor surface.")
    ...
    if state == "V3.9":
        ...
        if not exact_overlay:
            errors.append("Target claims V3.9 state but its overlay is incomplete or mismatched.")

    return {
        "errors": list(dict.fromkeys(errors)),
        "exact_overlay": exact_overlay,
        "immutable_v38_file_count": len(immutable),
        "immutable_v38_files_exact": (
        ...
        "partial_v39_paths": partial_paths,
...
def preflight(source_root: str | Path, target_root: str | Path) -> dict[str, Any]:
    ...
    if errors:
        return {
            ...
            "overlay_file_count": len(V39_TRANSITION_FILES),
            "source_commitments": {},
            "source_files": [],
            "target": {"valid": False},
            "valid": False,
...
def _verification_commands(root: Path) -> tuple[tuple[str, list[str]], ...]:
    return (
        (
            "v39_unit_tests",
            [sys.executable, "-m", "unittest", "quantum_research_lab.test_phase3_v39"],
        ),
        (
            "v39_release_tests",
            [
                ...
                "-m",
                "unittest",
                "quantum_research_lab.test_phase3_v39_release",
            ],
        ),
        (
            "v39_release_chain",
            [sys.executable, "-m", "quantum_research_lab.verify_phase3_v39", str(root)],
        ),
        (
            "v39_release_freeze",
            [
                sys.executable,
                "-m",
                "quantum_research_lab.verify_freeze_contract_v39",
                str(root),
                "--contract",
                str(root / V39_FREEZE_PATH),
            ],
        ),
        (
            "v39_streamlit_positive_rerun",
            [
                sys.executable,
                "-m",
                "quantum_research_lab.verify_phase3_v39_ui",
                str(root / "app_v39_offline_harness.py"),
                "--timeout",
                "180",
            ],
        ),
    ...
    *,
    ...
    for index, (label, command) in enumerate(commands):
        if force_failure and index == 0:
            raise RuntimeError("Injected V3.9 post-install verification fault.")
...
def _stage_candidate(
    ...
    frozen = freeze.get("frozen_files") or {}
    transition_payloads = set(V39_TRANSITION_FILES) - {V39_FREEZE_PATH}
    ...
    freeze_source = _contained(source_root, V39_FREEZE_PATH, must_exist=True)
    freeze_target = stage_root / V39_FREEZE_PATH
    shutil.copy2(freeze_source, freeze_target, follow_symlinks=False)
    staged_freeze = _read_json_strict(freeze_target)
    if _freeze_semantic(staged_freeze) != commitments.get(V39_FREEZE_PATH):
        raise RuntimeError("Candidate V3.9 freeze commitment mismatch.")
    for relative in V39_TRANSITION_FILES:
...
def _verify_staged_candidate(
    ...
    frozen = freeze.get("frozen_files") or {}
    if len(frozen) != EXPECTED_V39_FROZEN_FILE_COUNT:
    ...
    staged_freeze = _read_json_strict(
        _contained(stage_root, V39_FREEZE_PATH, must_exist=True)
    )
    if (
        _freeze_semantic(staged_freeze) != commitments.get(V39_FREEZE_PATH)
        or staged_freeze.get("freeze_contract_sha256")
        != commitments.get(V39_FREEZE_PATH)
    ):
        raise RuntimeError("Candidate V3.9 freeze changed after validation.")
...
def install(
    ...
    *,
    ...
    """Authenticate and optionally apply the V3.8 -> V3.9 transition."""
    ...
    try:
        ...
        if not report["valid"]:
            ...
            and report["target"].get("successor_state") == "V3.9"
            ...
            short = token[:12]
            stage_root = target / f".quantum-lab-v39-stage-{short}"
            backup_root = target / f".quantum-lab-v39-backup-{timestamp}-{short}"
            stage_root.mkdir(mode=0o700)
            backup_root.mkdir(mode=0o700)
            _fsync_directory(target)

            freeze = _read_json_strict(
                _contained(source, V39_FREEZE_PATH, must_exist=True)
            )
            ...
            try:
                for relative in V39_TRANSITION_FILES:
                    ...
                    if _test_fail_after is not None and len(touched) == _test_fail_after:
                        raise RuntimeError("Injected V3.9 partial-commit fault.")
                ...
                if not (
                    ...
                    and postinstall.get("successor_state") == "V3.9"
                ):
                    raise RuntimeError("Installed overlay failed exact V3.9 authentication.")
...
def _main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Safely install Quantum Lab V3.9.")
