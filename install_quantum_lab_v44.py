"""Transactional fail-closed installer for the Quantum Lab V4.4 overlay.

Only an exact V4.3 target or an exact V4.4 no-op state is accepted. The
...
INSTALLER_VERSION = "QUANTUM LAB V4.4 TRANSACTIONAL INSTALLER · V1"
LOCK_NAME = ".quantum-lab-v44-install.lock"
PARENT_LOCK_NAME = ".quantum-lab-v43-install.lock"
RESIDUAL_LOCK_NAMES = (PARENT_LOCK_NAME, ".quantum-lab-v42-install.lock")
PARENT_FREEZE_PATH = "FREEZE_CONTRACT_V4_3.json"
V44_FREEZE_PATH = "FREEZE_CONTRACT_V4_4.json"
README_PATH = "quantum_research_lab/README.md"
UI_PATH = "quantum_research_lab/ui.py"
PARENT_FREEZE_RAW_SHA256 = "950dc3eb7c9887bff7d7c6516942389fccb77f753291a158d18f407fea17efd7"
PARENT_FREEZE_SEMANTIC_SHA256 = "d058a2545e8a9e4788360663d36ae7f451e1fe126c290d60d173910b83441a49"
PARENT_FROZEN_FILE_COUNT = 193
PARENT_IMMUTABLE_FILE_COUNT = 191
PARENT_FROZEN_PATHS_FINGERPRINT = "3bc4c8805cf38d5e55c8cc4d1c99cfc3950f9b7faa2cb85fa891fb42987467fc"
EXPECTED_V44_FROZEN_FILE_COUNT = 211
EXPECTED_V44_FROZEN_PATHS_FINGERPRINT = "eb4fda2c2b862fa279a7f0d60f0a2694e50f9c63bf89f60de9d0af52638c0516"
...
V44_TRANSITION_FILES = (
    ...
    "outputs/quantum_phase3/v44_named_backend/SEALED_V4_4_NAMED_BACKEND_ZERO_JOB_ARTIFACT.json",
    ...
    "quantum_research_lab/phase3_v44_validation.py",
...
V44_ONLY_FILES = frozenset(V44_TRANSITION_FILES) - {README_PATH, UI_PATH}
...
def _source_inventory(source_root: Path) -> tuple[dict[str, Any], list[dict[str, Any]], list[str]]:
    errors: list[str] = []
    try:
        freeze_path = _contained(source_root, V44_FREEZE_PATH, must_exist=True)
        freeze = _read_json_strict(freeze_path)
    except Exception as exc:
        return {}, [], [f"Cannot read source V4.4 freeze: {exc}"]
    frozen = freeze.get("frozen_files") or {}
    if not isinstance(frozen, dict):
        return freeze, [], ["V4.4 frozen_files must be an object."]
    if not (
        freeze.get("frozen_file_count") == EXPECTED_V44_FROZEN_FILE_COUNT
        and len(frozen) == EXPECTED_V44_FROZEN_FILE_COUNT
        and _path_fingerprint(frozen) == EXPECTED_V44_FROZEN_PATHS_FINGERPRINT
        and freeze.get("freeze_contract_sha256") == _freeze_semantic(freeze)
    ):
        errors.append("V4.4 source freeze identity, count or path fingerprint mismatch.")
    policy = freeze.get("successor_policy") or {}
    if not (
        policy.get("accepted_target_states") == ["V4.3", "V4.4"]
        and policy.get("allowed_v43_superseded_files") == [README_PATH, UI_PATH]
        and policy.get("mixed_or_third_state") == "REJECT"
        and _valid_sha(policy.get("v44_successor_readme_sha256"))
        and _valid_sha(policy.get("v44_successor_ui_sha256"))
    ):
        errors.append("V4.4 source successor policy is invalid.")
    missing = set(V44_TRANSITION_FILES) - {V44_FREEZE_PATH} - set(frozen)
    if missing:
        errors.append("V4.4 freeze omits transition paths: " + ", ".join(sorted(missing)))
    if frozen.get(PARENT_FREEZE_PATH) != PARENT_FREEZE_RAW_SHA256:
        errors.append("V4.4 freeze does not preserve the exact V4.3 freeze file.")
...
def _successor_state(readme_sha: str | None, ui_sha: str | None, parent: Mapping[str, Any], source: Mapping[str, Any]) -> str:
    ...
    if pair == (parent_policy.get("v43_successor_readme_sha256"), parent_policy.get("v43_successor_ui_sha256")):
        return "V4.3"
    if pair == (source_policy.get("v44_successor_readme_sha256"), source_policy.get("v44_successor_ui_sha256")):
        return "V4.4"
    return "INVALID"
...
def authenticate_target(source_root: Path, target_root: Path) -> dict[str, Any]:
    errors: list[str] = []
    try:
        source_freeze = _read_json_strict(_contained(source_root, V44_FREEZE_PATH, must_exist=True))
        if source_freeze.get("freeze_contract_sha256") != _freeze_semantic(source_freeze):
            raise ValueError("V4.4 source freeze self-hash mismatch")
    except Exception as exc:
        source_freeze = {}
    ...
    try:
        ...
        errors.append(f"Cannot authenticate target V4.3 freeze: {exc}")
    ...
    if not parent_identity:
        errors.append("Target V4.3 freeze raw/semantic identity or inventory mismatch.")
    ...
    if isinstance(parent_files, dict):
        for relative in sorted(set(parent_files) - {README_PATH, UI_PATH}):
            ...
            if not immutable[relative]:
                errors.append(f"Target immutable V4.3 mismatch: {relative}")
    try:
    ...
    partial: list[str] = []
    if state == "V4.3":
        partial = sorted(relative for relative in V44_ONLY_FILES if (target_root / relative).exists() or (target_root / relative).is_symlink())
        if partial:
            errors.append("Partial V4.4 destinations exist on a V4.3 successor surface.")
    exact_overlay = False
    if state == "V4.4":
        exact_overlay = True
        ...
        if not exact_overlay:
            errors.append("Target claims V4.4 state but its overlay is incomplete or mismatched.")
    return {
        "errors": list(dict.fromkeys(errors)),
        "exact_overlay": exact_overlay,
        "immutable_v43_file_count": len(immutable),
        "immutable_v43_files_exact": len(immutable) == PARENT_IMMUTABLE_FILE_COUNT and all(immutable.values()),
...
def preflight(source_root: str | Path, target_root: str | Path) -> dict[str, Any]:
    ...
    if errors:
        return {"errors": errors, "installer_version": INSTALLER_VERSION, "overlay_file_count": len(V44_TRANSITION_FILES), "source_commitments": {}, "source_files": [], "target": {"valid": False}, "valid": False}
    ...
    return {
        ...
        "overlay_file_count": len(V44_TRANSITION_FILES),
...
def _verification_commands(root: Path) -> tuple[tuple[str, list[str]], ...]:
    return (
        ("v44_unit_tests", [sys.executable, "-m", "unittest", "quantum_research_lab.test_phase3_v44"]),
        ("v44_release_identity", [sys.executable, "-m", "quantum_research_lab.verify_phase3_v44", str(root), "--identity-only"]),
        ("v44_release_freeze", [sys.executable, "-m", "quantum_research_lab.verify_freeze_contract_v44", str(root)]),
        ("v44_streamlit_positive_rerun", [sys.executable, "-m", "quantum_research_lab.verify_phase3_v44_ui", str(root / "app_v44_offline_harness.py"), "--timeout", "600"]),
        ("historical_scientific_unit_regression", [sys.executable, "-m", "unittest", "quantum_research_lab.test_phase3_gate_compiler", "quantum_research_lab.test_phase3_algorithmic_contract", "quantum_research_lab.test_phase3_v34", "quantum_research_lab.test_phase3_v35", "quantum_research_lab.test_phase3_v36", "quantum_research_lab.test_phase3_v37", "quantum_research_lab.test_phase3_v38", "quantum_research_lab.test_phase3_v39", "quantum_research_lab.test_phase3_v40", "quantum_research_lab.test_phase3_v41", "quantum_research_lab.test_phase3_v42", "quantum_research_lab.test_phase3_v43"]),
    )
...
def _verify_release_identity_in_root(root: Path, label: str) -> dict[str, Any]:
    """Run the authenticated V4.4 verifier from the candidate root itself.
    ...
    command = [
        sys.executable,
        "-m",
        "quantum_research_lab.verify_phase3_v44",
        str(root),
        "--identity-only",
    ]
    completed = subprocess.run(
...
def _stage_candidate(
    ...
    frozen = freeze.get("frozen_files") or {}
    transition_payloads = set(V44_TRANSITION_FILES) - {V44_FREEZE_PATH}
    ...
    freeze_source = _contained(source_root, V44_FREEZE_PATH, must_exist=True)
    shutil.copy2(freeze_source, stage_root / V44_FREEZE_PATH, follow_symlinks=False)
...
def _rehash_source(source_root: Path, commitments: Mapping[str, str]) -> None:
    freeze = _read_json_strict(_contained(source_root, V44_FREEZE_PATH, must_exist=True))
...
def _rehash_transition_root(root: Path, commitments: Mapping[str, str], label: str) -> None:
    freeze = _read_json_strict(_contained(root, V44_FREEZE_PATH, must_exist=True))
    for relative in V44_TRANSITION_FILES:
...
def _reject_parent_lock(target_root: Path) -> None:
    ...
    if present:
        raise RuntimeError(
            "Residual predecessor installer lock rejected; inspect before V4.4 deployment: "
            + ", ".join(present)
        )
    ...
    for relative in reversed(tuple(touched_paths)):
        ...
        try:
            ...
            if installed is None or current != installed:
                ...
                raise RuntimeError(
                    f"External mutation detected after V4.4 write; refusing rollback overwrite: {relative}"
                )
            if expected["existed"]:
...
def apply_overlay(
    ...
    *,
    ...
    if force_commit_failure_after is not None and not 1 <= force_commit_failure_after <= len(V44_TRANSITION_FILES):
        return {
            "applied": False,
            "errors": ["force_commit_failure_after must identify one of the 20 ordered overlay writes."],
    ...
    token = uuid4().hex
    lock: Path | None = None
    backup_root = target / f".quantum-lab-v44-backup-{token}"
    stage_root = target / f".quantum-lab-v44-stage-{token}"
    report: dict[str, Any] = {}
    ...
    try:
        ...
        if (
            (report.get("target") or {}).get("successor_state") == "V4.4"
            and (report.get("target") or {}).get("exact_overlay") is True
        ):
            status = "NO_OP_ALREADY_EXACT_V4_4"
            valid = True
        else:
            ...
            backup_manifest = _write_audit_manifest(
                backup_root / "PREIMAGE_MANIFEST.json",
                {
                    ...
                    "format": "QUANTUM LAB V4.4 PREIMAGE MANIFEST · V1",
            ...
            freeze = _read_json_strict(_contained(source, V44_FREEZE_PATH, must_exist=True))
            ...
            intent_manifest = _write_audit_manifest(
                ...
                {
                    "created_utc": datetime.now(timezone.utc).isoformat(),
                    "format": "QUANTUM LAB V4.4 INSTALL INTENT · V1",
                    "installer": INSTALLER_VERSION,
            ...
            status = "COMMITTING"
            for index, relative in enumerate(V44_TRANSITION_FILES, start=1):
                target_path = target / relative
                ...
                if force_commit_failure_after == index:
                    raise RuntimeError(f"Injected V4.4 commit fault after ordered write {index}.")
        ...
        if touched_paths:
            ...
            if not rollback_report.get("preimages_restored"):
                ...
                rollback_report["predecessor_reauthenticated"] = bool(
                    ...
                    and predecessor.get("successor_state") == "V4.3"
                )
                if rollback_report["predecessor_reauthenticated"] is not True:
                    status = "ROLLBACK_INCOMPLETE"
                    errors.extend(predecessor.get("errors") or ["Restored V4.3 predecessor did not reauthenticate."])
    finally:
