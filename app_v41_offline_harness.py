"""Validation-only V4.1 AppTest harness with fail-fast provider spies.

The harness renders the real Quantum Lab and sealed local evidence while
...
counted independently across reruns.
"""
...
SPY_KEYS = {
    "provider": "v41_provider_spy_calls",
    "transpile": "v41_transpile_spy_calls",
    "seal": "v41_seal_spy_calls",
}
...
def _forbidden_call(kind: str) -> Callable[..., None]:
    def fail(*args, **kwargs) -> None:
        ...
        raise RuntimeError(f"V4.1 forbidden {kind} spy was invoked")
...
# Every script run reintroduces stale values. The BANDS lane must clear them on
# both the initial render and the positive rerun before provider controls exist.
...
ui.qpu_phase2_gate = lambda: {
    ...
    "reason": "Offline V4.1 UI fixture; excluded from scientific evidence.",
...
stale_provider_state_cleared = bool(
    ...
    and st.session_state.get("qrl_v221_p3_saved_account") is True
    and not any(key in st.session_state for key in opaque_stale_keys)
)
st.caption(
    "V4.1 OFFLINE APPTEST HARNESS · "
