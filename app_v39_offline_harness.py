"""Validation-only V3.9 AppTest harness with fail-fast provider spies."""
...
DYADIC = json.loads((ROOT / "SEALED_EXACT_DYADIC_BANDS_ORACLE.json").read_text(encoding="utf-8"))


def _forbidden_provider_call(*args, **kwargs):
    st.session_state["v39_provider_spy_calls"] = int(st.session_state.get("v39_provider_spy_calls", 0)) + 1
    raise RuntimeError("V3.9 provider/transpile/seal spy was invoked")


st.session_state.setdefault("v39_provider_spy_calls", 0)
...
ui.qpu_phase2_gate = lambda: {
    ...
    "reason": "Offline V3.9 UI fixture; excluded from scientific evidence.",
...
ui.bands_oracle_worker_status = lambda *args, **kwargs: {"status": "FROZEN", "selected_bits": None, "sealed": False}
...
ui.qpu_protocol_draft = lambda *args, **kwargs: {"status": "DRAFT · OFFLINE FIXTURE", "hardware_executable": False}
ui.qpu_discover_ibm_backends = _forbidden_provider_call
ui.qpu_transpile_probe = _forbidden_provider_call
ui.qpu_seal_protocol = _forbidden_provider_call

ui.render_quantum_research_lab(ticker="SPY", price_data=pd.DataFrame())

st.caption(
    f"V3.9 OFFLINE APPTEST HARNESS · PROVIDER SPY CALLS · {int(st.session_state.get('v39_provider_spy_calls', 0))} · "
