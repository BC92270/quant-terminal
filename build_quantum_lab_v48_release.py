"""Deterministic package and freeze builder for Quantum Lab V4.8."""
...
from quantum_research_lab.verify_freeze_contract_v48 import (
    ARTIFACT_PATH,
...
BUILDER_VERSION = "QUANTUM LAB V4.8 DETERMINISTIC RELEASE BUILDER · V1"
INSTITUTIONAL_ARCHIVE = "Quantum_Lab_V4_8_Institutional_Release.zip"
OVERLAY_ARCHIVE = "Quantum_Lab_V4_8_Deployment_Overlay.zip"
FIXED_ZIP_TIMESTAMP = (2026, 9, 21, 12, 0, 0)
MANIFEST_NAME = "QUANTUM_LAB_V4_8_RELEASE_MANIFEST.json"
REPORT_NAME = "QUANTUM_LAB_V4_8_RELEASE_REPORT.md"
CHECKSUM_NAME = "QUANTUM_LAB_V4_8_PACKAGE_SHA256.txt"
SOURCE_TREE_IDENTITY_NAME = "QUANTUM_LAB_V4_8_SOURCE_TREE_25_PATH_RAW_SHA256"
...
def _validation_api() -> Any:
    module = _module("quantum_research_lab.phase3_v48_validation")
    for attribute in (
        "EXPECTED_CHECK_COUNT",
        "run_v48_validation",
    ):
        if not hasattr(module, attribute):
            raise RuntimeError(f"V4.8 validation API missing {attribute}")
    return module


def _ui_api() -> Any:
    module = _module("quantum_research_lab.verify_phase3_v48_ui")
    if not hasattr(module, "verify_streamlit_surface_v48"):
        raise RuntimeError("V4.8 UI verifier API missing verify_streamlit_surface_v48")
    return module
...
def _load_validation_report(
    ...
    *,
    ...
    if not (
        ...
        == 96
        and scientific.get("independent_checker_check_count") == 65
        ...
        raise ValueError(
            "Exact V4.8 sealed validation/replay report is not bound to the "
...
def _run_validation(root: Path) -> dict[str, Any]:
    api = _validation_api()
    validation = api.run_v48_validation(root=root)
    ...
    if not (
        ...
        raise RuntimeError(f"V4.8 scientific validation failed before freeze: {validation}")
    return validation
    ...
    *,
    ...
    if not path.is_file():
        raise FileNotFoundError(
            "V4.8 validation report missing. Generate it explicitly with a fresh "
...
def _semantic_identity(root: Path, relative: str, field: str) -> str:
    ...
    if payload.get(field) != actual:
        raise ValueError(f"V4.8 semantic self-hash mismatch: {relative}")
    return actual
...
def _safe_claim_boundary(artifact: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(artifact_boundary, Mapping):
        raise ValueError("V4.8 artifact claim_boundary must be an object")
    for key in (
        "provider_calls",
        "network_calls",
        "backend_run_calls",
        ...
        if artifact_boundary.get(key) not in {None, 0}:
            raise ValueError(f"V4.8 artifact violates zero-operation boundary: {key}")
    if artifact_boundary.get("hardware_executable") not in {None, False}:
        raise ValueError("V4.8 artifact cannot be hardware executable")
    for key in ("provider_sdk_imported", "provider_credentials_read"):
        if artifact_boundary.get(key) not in {None, False}:
            raise ValueError(f"V4.8 artifact violates offline boundary: {key}")
    return {
...
def _ui_validation(root: Path) -> tuple[dict[str, Any], int]:
    ...
    if expected != 42:
        raise RuntimeError(
            f"V4.8 UI verifier check-count drifted: {expected} != 42"
        )
    report = api.verify_streamlit_surface_v48(
        root / "app_v48_offline_harness.py",
        timeout_seconds=600,
    )
    if report.get("valid") is not True or report.get("check_count") != expected:
        raise RuntimeError(f"V4.8 Streamlit validation failed before freeze: {report}")
...
def seal_freeze_contract(
    ...
    *,
    ...
    if V48_TRANSITION_ORDER_SHA256 != EXPECTED_V48_OVERLAY_ORDER_SHA256:
        raise AssertionError("V4.8 overlay order identity mismatch.")
    ...
    if not (
        ...
        raise ValueError("Exact V4.7 parent freeze authentication failed.")
    immutable_parent = {
        str(path): str(digest)
    ...
    if len(immutable_parent) != PARENT_IMMUTABLE_FILE_COUNT:
        raise ValueError("V4.7 immutable successor inventory must contain 270 files.")
    for relative, expected in immutable_parent.items():
        if _sha256(_safe_file(release_root, relative)) != expected:
            raise ValueError(f"V4.7 immutable parent mismatch: {relative}")
    ...
    if report_path != packaged_report_path:
        raise ValueError(
            "The V4.8 validation report must be the exact packaged transition path: "
    ...
    release_api = _module("quantum_research_lab.verify_phase3_v48")
    ...
    if release_chain.get("passed") is not True:
        raise RuntimeError(
            f"V4.8 scientific release-chain validation failed before freeze: {release_chain}"
        )
    ...
    if len(frozen) != EXPECTED_V48_FROZEN_FILE_COUNT or fingerprint != EXPECTED_V48_FROZEN_PATHS_FINGERPRINT:
        raise AssertionError(
            f"V4.8 freeze inventory drifted: count={len(frozen)} fingerprint={fingerprint}"
        )
    ...
    if release_chain.get("counts", {}).get("checks_total") != release_checks:
        raise RuntimeError("V4.8 release-chain check-count drift detected.")
    ...
    core: dict[str, Any] = {
        ...
        "deployment_contract": {
            ...
            "exact_v48_freeze_raw_bytes_required": True,
            "idempotent_exact_reapply": "NO_OP",
            "integration_surface_last": UI_PATH,
            "lock_release_requires_owned_token": True,
            "ordered_transition_paths": list(V48_TRANSITION_FILES),
            "ordered_transition_paths_sha256": EXPECTED_V48_OVERLAY_ORDER_SHA256,
        ...
        "freeze_contract_version": "QUANTUM LAB V4.8 FREEZE CONTRACT · V1",
        "frozen_file_count": len(frozen),
        "frozen_files": frozen,
        "frozen_paths_fingerprint_sha256": fingerprint,
        "lineage": {
            "append_only": True,
            "allowed_v47_superseded_files": [README_PATH, UI_PATH],
            "v47_freeze_raw_file_sha256": PARENT_FREEZE_RAW_SHA256,
            "v47_freeze_sha256": PARENT_FREEZE_SEMANTIC_SHA256,
            "v47_frozen_file_count": PARENT_FROZEN_FILE_COUNT,
            "v47_frozen_paths_fingerprint_sha256": PARENT_FROZEN_PATHS_FINGERPRINT,
            "v47_immutable_file_count": PARENT_IMMUTABLE_FILE_COUNT,
        },
        "packaging_contract": {
            ...
            "institutional_archive_entries": EXPECTED_V48_FROZEN_FILE_COUNT + 1,
            "overlay_archive_entries": len(V48_TRANSITION_FILES),
            "source_tree_raw_sha256_emitted_out_of_band": True,
        },
        "release_paths": list(V48_TRANSITION_FILES),
        ...
        "successor_policy": {
            "accepted_target_states": ["V4.7", "V4.8"],
            "allowed_v47_superseded_files": [README_PATH, UI_PATH],
            "immutable_v47_frozen_file_count": PARENT_IMMUTABLE_FILE_COUNT,
            "mixed_or_third_state": "REJECT",
            "v48_successor_readme_sha256": frozen[README_PATH],
            "v48_successor_ui_sha256": frozen[UI_PATH],
        },
        "v48_identities": identities,
        "validation_evidence": {
        ...
        "validation_targets": {
            ...
            "frozen_file_count": EXPECTED_V48_FROZEN_FILE_COUNT,
            "independent_scientific_checks": 65,
    ...
    target = release_root / V48_FREEZE_PATH
    target.write_text(
        json.dumps(
            contract,
            indent=2,
    ...
    commands = {
        "scientific_unit_tests": (
            ...
            [
                ...
                "-m",
                "unittest",
                "quantum_research_lab.test_phase3_v48",
                "-v",
            ],
        ),
        "release_hardening_tests": (
            ...
            [
                ...
                "-m",
                "unittest",
                "quantum_research_lab.test_phase3_v48_release",
                "-v",
            ],
        ),
    }
...
def _reauthenticate_release_source(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError(
            f"V4.8 source authentication failed at {phase}: "
    ...
    if (
        ...
        raise RuntimeError(
            f"V4.8 freeze verification failed at {phase}: {freeze_report}"
        )
...
def build_release(
    ...
    *,
    ...
    institutional_names = [V48_FREEZE_PATH] + sorted(contract["frozen_files"])
    overlay_names = list(V48_TRANSITION_FILES)
    if (
        len(institutional_names) != EXPECTED_V48_FROZEN_FILE_COUNT + 1
        or len(overlay_names) != len(V48_TRANSITION_FILES)
    ):
        raise AssertionError("V4.8 package entry count drifted.")
    with tempfile.TemporaryDirectory(prefix="quantum-v48-package.") as temporary:
        temp = Path(temporary)
    ...
    manifest_core = {
        ...
        "freeze_contract_raw_file_sha256": _sha256(release_root / V48_FREEZE_PATH),
        ...
        "validation_report_sha256": contract["v48_identities"]["validation_report_sha256"],
        "v48_artifact_raw_file_sha256": contract["v48_identities"]["artifact_raw_file_sha256"],
        "v48_artifact_sha256": contract["v48_identities"]["artifact_sha256"],
    }
    manifest = {
        **manifest_core,
    ...
    release_report_path.write_text(
        "# Quantum Lab V4.8 release report\n\n"
        ...
        "- Independent standard-library checks: 65/65\n"
