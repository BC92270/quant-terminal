"""Release-hardening tests for the Quantum Lab V3.9 -> V4.0 transition."""
...
from install_quantum_lab_v40 import (
    EXPECTED_V40_FROZEN_FILE_COUNT,
    EXPECTED_V40_FROZEN_PATHS_FINGERPRINT,
    LOCK_NAME,
...
V39_ARCHIVE = OUTPUT_ROOT / "Quantum_Lab_V3_9_Institutional_Release.zip"
V40_FULL_ARCHIVE = OUTPUT_ROOT / "Quantum_Lab_V4_0_Institutional_Release.zip"
V40_OVERLAY_ARCHIVE = OUTPUT_ROOT / "Quantum_Lab_V4_0_Deployment_Overlay.zip"

EXPECTED_TRANSITION_FILES = (
    "FREEZE_CONTRACT_V4_0.json",
    "DEPLOY_V4_0.md",
...
def _transition_snapshot(root: Path) -> dict[str, bytes | None]:
    return {
        ...
        for relative in V40_TRANSITION_FILES
    }


class InstallerV40StaticContractTests(unittest.TestCase):
    def test_overlay_is_exact_unique_and_in_dependency_order(self) -> None:
        self.assertEqual(V40_TRANSITION_FILES, EXPECTED_TRANSITION_FILES)
        self.assertEqual(len(V40_TRANSITION_FILES), 18)
        self.assertEqual(len(V40_TRANSITION_FILES), len(set(V40_TRANSITION_FILES)))
        self.assertEqual(V40_TRANSITION_FILES[-2:], (README_PATH, UI_PATH))
        self.assertEqual(V40_TRANSITION_FILES[0], V40_FREEZE_PATH)

    def test_v40_inventory_constants_are_final(self) -> None:
        self.assertEqual(EXPECTED_V40_FROZEN_FILE_COUNT, 145)
        self.assertEqual(
            EXPECTED_V40_FROZEN_PATHS_FINGERPRINT,
            EXPECTED_V40_FINGERPRINT,
        )

    def test_exact_parent_v39_constants_match_local_parent(self) -> None:
        parent_path = ROOT / "FREEZE_CONTRACT_V3_9.json"
    ...
    def test_duplicate_nonfinite_traversal_and_symlink_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory(prefix="quantum-v40-strict-") as temporary:
            root = Path(temporary)
    ...
    def test_foreign_lock_is_preserved(self) -> None:
        with tempfile.TemporaryDirectory(prefix="quantum-v40-lock-") as temporary:
...
class FrozenV40ReleaseTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        required = (
            ROOT / V40_FREEZE_PATH,
            ROOT / "install_quantum_lab_v40.py",
            ROOT / "quantum_research_lab/verify_phase3_v40.py",
            ROOT / "quantum_research_lab/verify_freeze_contract_v40.py",
        )
        if not all(path.is_file() for path in required):
            raise unittest.SkipTest(
                "V4.0 freeze, installer and release verifiers are not assembled yet."
            )
        cls.freeze = _read_json_strict(ROOT / V40_FREEZE_PATH)

    def _copy_release(self, destination: Path) -> None:
        for relative in [*self.freeze["frozen_files"], V40_FREEZE_PATH]:
            source = ROOT / relative
            target = destination / relative
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(source, target)

    def _extract_v39(self, destination: Path) -> None:
        if not V39_ARCHIVE.is_file():
            self.skipTest("Institutional V3.9 predecessor archive is unavailable.")
        _safe_extract(V39_ARCHIVE, destination)

    def test_release_chain_and_freeze_pass(self) -> None:
        from .verify_freeze_contract_v40 import verify_freeze_contract
        from .verify_phase3_v40 import verify_release_chain

        release = verify_release_chain(ROOT)
        freeze = verify_freeze_contract(ROOT, ROOT / V40_FREEZE_PATH)
    ...
    def test_inventory_self_hash_and_all_raw_hashes_are_exact(self) -> None:
        ...
        self.assertEqual(len(frozen), EXPECTED_V40_FROZEN_FILE_COUNT)
        self.assertEqual(
            self.freeze["frozen_file_count"], EXPECTED_V40_FROZEN_FILE_COUNT
        )
        self.assertEqual(
            _path_fingerprint(frozen), EXPECTED_V40_FROZEN_PATHS_FINGERPRINT
        )
        semantic = _canonical_json_sha256(
            {
                key: value
        ...
        self.assertTrue(
            set(V40_TRANSITION_FILES) - {V40_FREEZE_PATH} <= set(frozen)
        )
        self.assertEqual(
            frozen["FREEZE_CONTRACT_V3_9.json"], PARENT_FREEZE_RAW_SHA256
        )
        self.assertNotIn(V40_FREEZE_PATH, frozen)

    def test_clean_extract_is_self_contained(self) -> None:
        from .verify_freeze_contract_v40 import verify_freeze_contract
        from .verify_phase3_v40 import verify_release_chain

        with tempfile.TemporaryDirectory(prefix="quantum-v40-clean-") as temporary:
            destination = Path(temporary)
            self._copy_release(destination)
            release = verify_release_chain(destination)
            freeze = verify_freeze_contract(
                destination, destination / V40_FREEZE_PATH
            )
            self.assertTrue(release["valid"], release)
            self.assertTrue(freeze["valid"], freeze)
            ...
            isolated = subprocess.run(
                [sys.executable, "-m", "quantum_research_lab.verify_phase3_v40", "."],
                cwd=destination,
                env=environment,
                capture_output=True,
                text=True,
    ...
    def test_exact_reapplication_is_noop(self) -> None:
        with tempfile.TemporaryDirectory(prefix="quantum-v40-noop-") as temporary:
            destination = Path(temporary)
            self._copy_release(destination)
            ...
            self.assertFalse(report["applied"])
            self.assertEqual(report["postinstall"]["successor_state"], "V4.0")
    ...
    def test_unknown_or_mixed_successor_surface_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory(prefix="quantum-v40-mixed-") as temporary:
            destination = Path(temporary)
            self._copy_release(destination)
    ...
    def test_symlinked_overlay_destination_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory(prefix="quantum-v40-link-") as temporary:
            destination = Path(temporary)
            self._copy_release(destination)
            victim = destination / "DEPLOY_V4_0.md"
            victim.unlink()
            victim.symlink_to(destination / V40_FREEZE_PATH)
            report = preflight(ROOT, destination)
            self.assertFalse(report["valid"])
            self.assertTrue(
                any(
    ...
    def test_partial_v40_overlay_on_v39_surface_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory(prefix="quantum-v40-partial-") as temporary:
            destination = Path(temporary)
            self._extract_v39(destination)
            relative = "DEPLOY_V4_0.md"
            self.assertIn(relative, V40_ONLY_FILES)
            source = ROOT / relative
            target = destination / relative
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(source, target)
            report = preflight(ROOT, destination)
            self.assertFalse(report["valid"])
            self.assertEqual(report["target"]["successor_state"], "V3.9")
            self.assertIn(relative, report["target"]["partial_v40_paths"])

    def test_staged_source_race_is_rejected_before_commit(self) -> None:
        with tempfile.TemporaryDirectory(prefix="quantum-v40-source-race-") as temporary:
            base = Path(temporary)
            source = base / "source"
            target = base / "target"
            source.mkdir()
    ...
    def test_target_race_after_preimages_is_rejected_before_commit(self) -> None:
        with tempfile.TemporaryDirectory(prefix="quantum-v40-target-race-") as temporary:
            target = Path(temporary)
            self._extract_v39(target)
            ...
            self.assertTrue(
                all(
                    ...
                    for relative in V40_ONLY_FILES
                )
            )
            self.assertFalse((target / LOCK_NAME).exists())

    def test_partial_commit_faults_restore_exact_v39_preimages(self) -> None:
        self.assertEqual(len(V40_TRANSITION_FILES), 18)
        for failure_index in (1, 16, 18):
            with self.subTest(failure_index=failure_index):
                with tempfile.TemporaryDirectory(
                    prefix="quantum-v40-rollback-"
                ) as temporary:
                    destination = Path(temporary)
                    self._extract_v39(destination)
                    before = _transition_snapshot(destination)
                    ...
                    self.assertEqual(
                        report["rollback"]["successor_state"], "V3.9"
                    )
                    self.assertEqual(before, after)
                    self.assertFalse((destination / LOCK_NAME).exists())

    def test_real_v39_archive_to_v40_apply_then_reapply_is_noop(self) -> None:
        with tempfile.TemporaryDirectory(prefix="quantum-v40-e2e-") as temporary:
            destination = Path(temporary)
            self._extract_v39(destination)
            initial = preflight(ROOT, destination)
            self.assertTrue(initial["valid"], initial)
            self.assertEqual(initial["target"]["successor_state"], "V3.9")
            ...
            self.assertEqual(applied["applied_files"], list(V40_TRANSITION_FILES))
            self.assertTrue(applied["readme_committed_penultimate"])
            self.assertTrue(applied["ui_committed_last"])
            self.assertEqual(applied["postinstall"]["successor_state"], "V4.0")
            ...
            self.assertEqual(replay["postinstall"]["successor_state"], "V4.0")
            self.assertFalse((destination / LOCK_NAME).exists())

    def test_built_archives_have_exact_safe_inventories(self) -> None:
        if not (V40_FULL_ARCHIVE.is_file() and V40_OVERLAY_ARCHIVE.is_file()):
            self.skipTest("V4.0 deployment archives are not built yet.")

        with zipfile.ZipFile(V40_FULL_ARCHIVE) as full:
            full_names = full.namelist()
            self.assertIsNone(full.testzip())
            for name in full_names:
                self.assertEqual(full.read(name), (ROOT / name).read_bytes(), name)

        with zipfile.ZipFile(V40_OVERLAY_ARCHIVE) as overlay:
        ...
        self.assertEqual(len(full_names), 146)
        self.assertEqual(len(full_names), len(set(full_names)))
        self.assertEqual(
            set(full_names), {*self.freeze["frozen_files"], V40_FREEZE_PATH}
        )
        self.assertEqual(len(overlay_names), 18)
        self.assertEqual(overlay_names, list(V40_TRANSITION_FILES))
