"""Positive-rerun AppTest checks for the standalone V4.9 surface."""
...
def _text(app: AppTest) -> str:
    parts: list[str] = []
    for name in ("title", "header", "markdown", "caption", "warning", "error", "info", "success", "code", "metric"):
...
class QuantumLabV49AppTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        target = ROOT / "release/quantum_v49/harness.py"
    ...
    def test_03_six_tabs_are_stable(self) -> None:
        expected = ("DECISION", "EPOCH REGISTRY", "RESOURCE GAP", "CELL MATRIX", "PROVENANCE", "GOVERNANCE")
        self.assertEqual(self.first_tabs, expected)
        self.assertEqual(self.final_tabs, expected)

    def test_04_terminal_decision_is_visible(self) -> None:
        self.assertIn("V49_NOT_EVALUABLE_INSUFFICIENT_AUTHENTIC_EPOCHS", self.final_text)

    def test_05_research_boundary_is_visible(self) -> None:
        self.assertIn("RESEARCH_ONLY", self.final_text)
        self.assertIn("V5 · CLOSED", self.final_text)

    def test_06_six_metrics_are_rendered(self) -> None:
        self.assertEqual(len(self.app.metric), 6)

    def test_07_five_downloads_are_rendered(self) -> None:
        self.assertEqual(len(self.app.get("download_button")), 5)
    ...
    def test_09_epoch_boundary_is_visible(self) -> None:
        self.assertIn("1 / 3", self.final_text)
        self.assertIn("two additional authentic epochs", self.final_text)

    def test_10_resource_target_is_visible(self) -> None:
        self.assertIn("963", self.final_text)
        self.assertIn("838,686", self.final_text)
