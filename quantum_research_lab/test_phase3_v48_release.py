"""Release, packaging and transactional hardening tests for Quantum Lab V4.8."""
...
from build_quantum_lab_v48_release import (
    EXPECTED_RELEASE_HARDENING_TEST_COUNT,
    _load_validation_report,
    _write_deterministic_zip,
)
import install_quantum_lab_v48 as installer
...
from .verify_freeze_contract_v48 import (
    ARTIFACT_PATH,
    EXPECTED_FREEZE_CHECK_COUNT,
    VALIDATION_REPORT_PATH,
    verify_freeze_contract,
)
from .verify_phase3_v48 import (
...
V47_ARCHIVE = Path(
    os.environ.get(
        "QUANTUM_LAB_V47_ARCHIVE",
        str(EXTERNAL_OUTPUTS / "Quantum_Lab_V4_7_Institutional_Release.zip"),
    )
).expanduser().resolve()
EXPECTED_V47_ARCHIVE_RAW_SHA256 = (
    "9724710ada013508e0b6af156841313532d3e09d889ae2120ca9e68598d1e9f4"
)
...
def _extract_archive_safely(archive_path: Path, destination: Path) -> Path:
    if not archive_path.is_file():
        raise AssertionError(f"Exact V4.7 institutional fixture unavailable: {archive_path}")
    observed_sha256 = hashlib.sha256(archive_path.read_bytes()).hexdigest()
    if observed_sha256 != EXPECTED_V47_ARCHIVE_RAW_SHA256:
        raise AssertionError(
            "Exact V4.7 institutional fixture identity mismatch: "
            f"{observed_sha256}"
        )
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        for info in infos:
            ...
            if (
                ...
                raise AssertionError(f"Unsafe V4.7 fixture member: {info.filename}")
        archive.extractall(destination)
    return destination


def _v47_target(destination: Path) -> Path:
    return _extract_archive_safely(V47_ARCHIVE, destination)


def _v48_target(destination: Path) -> Path:
    freeze = read_json_strict(ROOT / V48_FREEZE_PATH)
    destination.mkdir(parents=True, exist_ok=True)
    for relative in freeze["frozen_files"]:
        _copy_file(ROOT / relative, destination / relative)
    _copy_file(ROOT / V48_FREEZE_PATH, destination / V48_FREEZE_PATH)
    return destination


def _sparse_v48_source(destination: Path) -> Path:
    destination.mkdir(parents=True, exist_ok=True)
...
def _fake_candidate(*_args: object, **_kwargs: object) -> tuple[Path, dict[str, object]]:
    stage = Path(tempfile.mkdtemp(prefix="quantum-v48-test-candidate."))
    return stage, {
        "passed": True,
...
class QuantumLabV48ReleaseTests(unittest.TestCase):
    def test_01_transition_inventory_is_exactly_ordered(self) -> None:
        self.assertEqual(len(V48_TRANSITION_FILES), 25)
        observed = hashlib.sha256(
            json.dumps(list(V48_TRANSITION_FILES), separators=(",", ":")).encode()
        ).hexdigest()
        self.assertEqual(observed, EXPECTED_V48_OVERLAY_ORDER_SHA256)

    def test_02_readme_and_ui_are_committed_last(self) -> None:
        self.assertEqual(V48_TRANSITION_FILES[-2:], (README_PATH, UI_PATH))
    ...
    def test_03_freeze_self_hash_count_and_fingerprint_are_exact(self) -> None:
        ...
        self.assertEqual(freeze["frozen_file_count"], EXPECTED_V48_FROZEN_FILE_COUNT)
        self.assertEqual(fingerprint, EXPECTED_V48_FROZEN_PATHS_FINGERPRINT)

    def test_04_source_inventory_authenticates_every_overlay_path(self) -> None:
        freeze, rows, errors = _source_inventory(ROOT)
        self.assertFalse(errors, errors)
        self.assertEqual(freeze["frozen_file_count"], EXPECTED_V48_FROZEN_FILE_COUNT)
        self.assertEqual(len(rows), len(V48_TRANSITION_FILES))
    ...
    def test_09_validation_report_is_self_hashed_and_byte_exact(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            artifact = read_json_strict(ROOT / ARTIFACT_PATH)
            scientific = run_v48_validation(root=ROOT)
    ...
    def test_10_artifact_identities_are_retained_in_freeze(self) -> None:
        freeze = read_json_strict(ROOT / V48_FREEZE_PATH)
        artifact = read_json_strict(ROOT / ARTIFACT_PATH)
        identities = freeze["v48_identities"]
        self.assertEqual(
    ...
    def test_11_deterministic_zip_double_build_is_identical(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            names = (V48_FREEZE_PATH, "DEPLOY_V4_8.md")
    ...
    def test_12_duplicate_archive_members_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            with self.assertRaises(ValueError):
                _write_deterministic_zip(
                    ROOT,
                    (V48_FREEZE_PATH, V48_FREEZE_PATH),
    ...
    def test_14_symlinked_archive_source_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            root.mkdir()
            os.symlink(ROOT / "DEPLOY_V4_8.md", root / "linked.md")
            with self.assertRaises(ValueError):
    ...
    def test_16_exact_v48_target_is_a_valid_no_op(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            target = _v48_target(Path(temporary) / "target")
    ...
    def test_17_exact_v48_target_tamper_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            target = _v48_target(Path(temporary) / "target")
            (target / "DEPLOY_V4_8.md").write_text("tampered", encoding="utf-8")
            report = preflight(
                ROOT,
                target,
                expected_source_tree_raw_sha256=_source_pin(ROOT),
        ...
        with tempfile.TemporaryDirectory() as temporary:
            target = _v48_target(Path(temporary) / "target")
            freeze_path = target / V48_FREEZE_PATH
            ...
            self.assertTrue(
                any(
                    f"Exact V4.8 target mismatch: {V48_FREEZE_PATH}" in error
    ...
    def test_18_symlinked_source_member_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            victim = source / "DEPLOY_V4_8.md"
            victim.unlink()
            os.symlink(ROOT / "DEPLOY_V4_8.md", victim)
    ...
    def test_19_tampered_source_member_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (source / "DEPLOY_V4_8.md").write_text("tampered", encoding="utf-8")
            _, _, errors = _source_inventory(source)
            ...
            # its validator.
            freeze_path = source / V48_FREEZE_PATH
            freeze = read_json_strict(freeze_path)
            freeze["frozen_files"]["DEPLOY_V4_8.md"] = hashlib.sha256(
                (source / "DEPLOY_V4_8.md").read_bytes()
            ).hexdigest()
            ...
            target = _v47_target(base / "target")
            report = preflight(
                source,
                target,
    ...
    def test_20_partial_v48_state_on_v47_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            target = _v47_target(Path(temporary) / "target")
            _copy_file(ROOT / "DEPLOY_V4_8.md", target / "DEPLOY_V4_8.md")
            ...
            self.assertFalse(report["valid"])
            self.assertIn("DEPLOY_V4_8.md", report["partial_v48_paths"])

    def test_21_all_25_commit_fault_positions_restore_exact_v47(self) -> None:
        with mock.patch.object(installer, "_build_candidate", side_effect=_fake_candidate):
            for position in range(1, len(V48_TRANSITION_FILES) + 1):
                with self.subTest(position=position), tempfile.TemporaryDirectory() as temporary:
                    ...
                    self.assertEqual(restored["state"], "V4.7")
            ...
            with self.subTest(phase="partial_commit_copy"), tempfile.TemporaryDirectory() as temporary:
                ...
                def fail_partial_commit_copy(source: Path, destination: Path, *args: object, **kwargs: object) -> Path:
                    ...
                    if ".v48-" in target_path.name and target_path.name.endswith(".tmp"):
            ...
            with self.subTest(phase="partial_rollback_replace"), tempfile.TemporaryDirectory() as temporary:
                ...
                with mock.patch.object(
                    ...
                    report = apply_overlay(
                        ...
                        target,
                        expected_source_tree_raw_sha256=source_pin,
                        fault_after=len(V48_TRANSITION_FILES),
                    )
                self.assertFalse(report["valid"], report)
    ...
    def test_22_all_phase_faults_restore_exact_v47(self) -> None:
        with mock.patch.object(installer, "_build_candidate", side_effect=_fake_candidate):
    ...
    def test_23_exact_v47_target_promotes_with_real_candidate_validation(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            target = _v47_target(Path(temporary) / "target")
            source_pin = _source_pin(ROOT)
            report = apply_overlay(
    ...
    def test_25_residual_predecessor_lock_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary, mock.patch.object(
            ...
            target = _v47_target(Path(temporary) / "target")
            (target / ".quantum-lab-v47-install.lock").mkdir()
            report = apply_overlay(
                ROOT,
                target,
                expected_source_tree_raw_sha256=_source_pin(ROOT),
    ...
    def test_28_installer_has_no_provider_or_network_client_import(self) -> None:
        ...
        builder_source = (ROOT / "build_quantum_lab_v48_release.py").read_text(
            encoding="utf-8"
        )
        ...
        tree = ast.parse((ROOT / "install_quantum_lab_v48.py").read_text(encoding="utf-8"))
        modules = set()
    ...
    def test_29_sparse_deployment_overlay_preflights_applies_and_no_ops(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            base = Path(temporary)
            source = _sparse_v48_source(base / "source")
            target = _v47_target(base / "target")
