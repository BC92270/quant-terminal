"""Fail-closed scientific release-chain verifier for Quantum Lab V4.7.

Identity mode authenticates the sealed result and its non-execution boundary.
Deep mode runs the independent 96-check validation.  Rebuild mode additionally
executes the offline optimizer in a clean process and requires byte-for-byte
equality.  No mode contacts a provider or submits a simulator/QPU job.
"""
...
from .phase3_v47_independent_checker import ARTIFACT_VERSION, EXPECTED_SEEDS, EXPECTED_WIDTHS
...
def _clean_rebuild(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="quantum-v47-release-replay.") as temporary:
        ...
        if completed.returncode != 0 or not output.is_file():
            return {
                "errors": [completed.stderr or completed.stdout or "V4.7 rebuild failed."],
                "performed": True,
        ...
        return {
            ...
            "errors": [] if valid else ["Clean-process V4.7 replay identity mismatch."],
            "performed": True,
            "returncode": completed.returncode,
            "valid": valid,
        }
...
def verify_release_chain(
    ...
    *,
    ...
    """Verify the V4.7 scientific chain without depending on a freeze file."""
    ...
    raw_pin = expected_artifact_raw_sha256 or EXPECTED_ARTIFACT_RAW_SHA256
    semantic_pin = expected_artifact_sha256 or EXPECTED_ARTIFACT_SHA256
    ...
    try:
        ...
        artifact = {}
        errors.append(f"V4.7 release inputs unavailable: {exc}")
    ...
    if deep:
        try:
            ...
            errors.append(f"V4.7 deep validation failed: {exc}")
    else:
        scientific = {
            "counts": {"checks_passed": 0, "checks_total": EXPECTED_SCIENTIFIC_CHECK_COUNT},
            "errors": [],
    ...
    checks: dict[str, bool] = {
        ...
        "source_crosslink_exact": bool(
            source_path.is_file()
            and artifact.get("source_raw_file_sha256") == raw_file_sha256(source_path)
        ),
        "checker_crosslink_exact": bool(
            checker_path.is_file()
            and artifact.get("independent_checker_raw_file_sha256") == raw_file_sha256(checker_path)
        ),
        "research_classification_exact": artifact.get("research_classification") == "RESEARCH_ONLY" and boundary.get("research_classification") == "RESEARCH_ONLY",
        ...
        "hardware_execution_rejected": boundary.get("hardware_executable") is False,
        "snapshot_not_current_hardware_evidence": boundary.get("snapshot_is_current_hardware_evidence") is False,
        ...
        "all_baseline_commitments_exact": len(rows) == 8 and aggregate.get("all_eight_baseline_commitments_exact_v46") is True and all(row.get("baseline_v46_commitments_exact") is True for row in rows),
        "deep_scientific_validation": scientific.get("passed") is True if deep else True,
        "deep_scientific_check_count": (
            _mapping(scientific.get("counts")).get("checks_passed") == EXPECTED_SCIENTIFIC_CHECK_COUNT
            and _mapping(scientific.get("counts")).get("checks_total") == EXPECTED_SCIENTIFIC_CHECK_COUNT
        ) if deep else True,
        "optional_clean_rebuild_byte_exact": rebuild_report.get("valid") is True,
    }
    if len(checks) != EXPECTED_RELEASE_CHECK_COUNT:
        raise AssertionError(
            f"V4.7 release verification check count drifted: {len(checks)} != {EXPECTED_RELEASE_CHECK_COUNT}"
        )
    ...
    return {
        ...
        "verifier": "QUANTUM LAB V4.7 SCIENTIFIC RELEASE-CHAIN VERIFIER · V1",
    }
