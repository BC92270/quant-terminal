"""Transactional, fail-closed installer for the Quantum Lab V4.0 overlay.

The only accepted transition is an exact V3.9 release to an exact V4.0
...
INSTALLER_VERSION = "QUANTUM LAB V4.0 TRANSACTIONAL INSTALLER · V1"
LOCK_NAME = ".quantum-lab-v40-install.lock"
PARENT_FREEZE_PATH = "FREEZE_CONTRACT_V3_9.json"
V40_FREEZE_PATH = "FREEZE_CONTRACT_V4_0.json"
README_PATH = "quantum_research_lab/README.md"
UI_PATH = "quantum_research_lab/ui.py"

PARENT_FREEZE_RAW_SHA256 = (
    "e44295aaab7689be7090ff6022a67e2e7d96cc40fcdaedf0328aa50294880c59"
)
PARENT_FREEZE_SEMANTIC_SHA256 = (
    "510790b4682e09376feb226b9d0b8b0046477ea456abf0a38e0cadb2410300ac"
)
PARENT_FROZEN_FILE_COUNT = 129
PARENT_IMMUTABLE_FILE_COUNT = 127
PARENT_FROZEN_PATHS_FINGERPRINT = (
    "ed2f8d6027b7b00c4ad68ed48f72e73b81dd240238849daba4c79ea050f72ce7"
)
PARENT_README_SHA256 = (
    "ad5ba3b0fc55be58b8ac7bf8e4a74613031816408f0ecbe0a888aebd09f8098c"
)
PARENT_UI_SHA256 = (
    "c2fc4b537e6dce5159720c5e8db9acc5376fc428fa61785417466a1b4928a67a"
)

EXPECTED_V40_FROZEN_FILE_COUNT = 145
...
V40_ONLY_FILES = frozenset(V40_TRANSITION_FILES) - {README_PATH, UI_PATH}
...
def _source_inventory(
    ...
    errors: list[str] = []
    try:
        freeze_path = _contained(source_root, V40_FREEZE_PATH, must_exist=True)
        freeze = _read_json_strict(freeze_path)
    except Exception as exc:
        return {}, [], [f"Cannot read source V4.0 freeze: {exc}"]
    frozen = freeze.get("frozen_files") or {}
    if not isinstance(frozen, dict):
        return freeze, [], ["V4.0 frozen_files must be an object."]
    if not (
        freeze.get("frozen_file_count") == EXPECTED_V40_FROZEN_FILE_COUNT
        and len(frozen) == EXPECTED_V40_FROZEN_FILE_COUNT
        and _path_fingerprint(frozen) == EXPECTED_V40_FROZEN_PATHS_FINGERPRINT
        and freeze.get("freeze_contract_sha256") == _freeze_semantic(freeze)
    ):
        errors.append("V4.0 source freeze identity, count, or path fingerprint mismatch.")
    policy = freeze.get("successor_policy") or {}
    if not (
        policy.get("accepted_target_states") == ["V3.9", "V4.0"]
        and policy.get("allowed_v39_superseded_files") == [README_PATH, UI_PATH]
        and policy.get("mixed_or_third_state") == "REJECT"
        and _valid_sha256(policy.get("v40_successor_readme_sha256"))
        and _valid_sha256(policy.get("v40_successor_ui_sha256"))
    ):
        errors.append("V4.0 source successor policy is invalid.")
    missing_from_freeze = set(V40_TRANSITION_FILES) - {V40_FREEZE_PATH} - set(frozen)
    if missing_from_freeze:
        errors.append(
            "V4.0 freeze omits transition paths: " + ", ".join(sorted(missing_from_freeze))
        )
    if frozen.get(PARENT_FREEZE_PATH) != PARENT_FREEZE_RAW_SHA256:
        errors.append("V4.0 freeze does not preserve the exact V3.9 freeze file.")

    rows: list[dict[str, Any]] = []
    for relative in V40_TRANSITION_FILES:
        try:
            path = _contained(source_root, relative, must_exist=True)
            if relative == V40_FREEZE_PATH:
...
def _successor_state(
    ...
    successor = (
        policy.get("v40_successor_readme_sha256"),
        policy.get("v40_successor_ui_sha256"),
    )
...
def authenticate_target(source_root: Path, target_root: Path) -> dict[str, Any]:
    ...
    try:
        source_freeze = _read_json_strict(
            _contained(source_root, V40_FREEZE_PATH, must_exist=True)
        )
        if source_freeze.get("freeze_contract_sha256") != _freeze_semantic(source_freeze):
            raise ValueError("V4.0 source freeze self-hash mismatch")
    except Exception as exc:
        source_freeze = {}
    ...
    try:
        ...
        errors.append(f"Cannot authenticate target V3.9 freeze: {exc}")
    ...
    if not parent_identity:
        errors.append("Target V3.9 freeze raw/semantic identity or inventory mismatch.")
    ...
    if isinstance(parent_files, dict):
        for relative in sorted(set(parent_files) - {README_PATH, UI_PATH}):
            ...
            if not immutable[relative]:
                errors.append(f"Target immutable V3.9 mismatch: {relative}")
    ...
    if state == "V3.9":
        ...
        if partial_paths:
            errors.append("Partial V4.0 destinations exist on a V3.9 successor surface.")
    ...
    if state == "V4.0":
        ...
        if not exact_overlay:
            errors.append("Target claims V4.0 state but its overlay is incomplete or mismatched.")

    return {
        "errors": list(dict.fromkeys(errors)),
        "exact_overlay": exact_overlay,
        "immutable_v39_file_count": len(immutable),
        "immutable_v39_files_exact": (
        ...
        "partial_v40_paths": partial_paths,
...
def preflight(source_root: str | Path, target_root: str | Path) -> dict[str, Any]:
    ...
    if errors:
        return {
            ...
            "overlay_file_count": len(V40_TRANSITION_FILES),
            "source_commitments": {},
            "source_files": [],
            "target": {"valid": False},
            "valid": False,
...
def _verification_commands(root: Path) -> tuple[tuple[str, list[str]], ...]:
    return (
        (
            "v40_unit_tests",
            [sys.executable, "-m", "unittest", "quantum_research_lab.test_phase3_v40"],
        ),
        (
            "v40_release_tests",
            [
                ...
                "-m",
                "unittest",
                "quantum_research_lab.test_phase3_v40_release",
            ],
        ),
        (
            "v40_release_chain",
            [sys.executable, "-m", "quantum_research_lab.verify_phase3_v40", str(root)],
        ),
        (
            "v40_release_freeze",
            [
                sys.executable,
                "-m",
                "quantum_research_lab.verify_freeze_contract_v40",
                str(root),
                "--contract",
                str(root / V40_FREEZE_PATH),
            ],
        ),
        (
            "v40_streamlit_positive_rerun",
            [
                sys.executable,
                "-m",
                "quantum_research_lab.verify_phase3_v40_ui",
                str(root / "app_v40_offline_harness.py"),
                "--timeout",
                "180",
            ],
        ),
    ...
    *,
    ...
    for index, (label, command) in enumerate(commands):
        if force_failure and index == 0:
            raise RuntimeError("Injected V4.0 post-install verification fault.")
...
def _stage_candidate(
    ...
    frozen = freeze.get("frozen_files") or {}
    transition_payloads = set(V40_TRANSITION_FILES) - {V40_FREEZE_PATH}
    ...
    freeze_source = _contained(source_root, V40_FREEZE_PATH, must_exist=True)
    freeze_target = stage_root / V40_FREEZE_PATH
    shutil.copy2(freeze_source, freeze_target, follow_symlinks=False)
    staged_freeze = _read_json_strict(freeze_target)
    if _freeze_semantic(staged_freeze) != commitments.get(V40_FREEZE_PATH):
        raise RuntimeError("Candidate V4.0 freeze commitment mismatch.")
    for relative in V40_TRANSITION_FILES:
...
def _verify_staged_candidate(
    ...
    frozen = freeze.get("frozen_files") or {}
    if len(frozen) != EXPECTED_V40_FROZEN_FILE_COUNT:
    ...
    staged_freeze = _read_json_strict(
        _contained(stage_root, V40_FREEZE_PATH, must_exist=True)
    )
    if (
        _freeze_semantic(staged_freeze) != commitments.get(V40_FREEZE_PATH)
        or staged_freeze.get("freeze_contract_sha256")
        != commitments.get(V40_FREEZE_PATH)
    ):
        raise RuntimeError("Candidate V4.0 freeze changed after validation.")
...
def install(
    ...
    *,
    ...
    """Authenticate and optionally apply the V3.9 -> V4.0 transition."""
    ...
    try:
        ...
        if not report["valid"]:
            ...
            and report["target"].get("successor_state") == "V4.0"
            ...
            short = token[:12]
            stage_root = target / f".quantum-lab-v40-stage-{short}"
            backup_root = target / f".quantum-lab-v40-backup-{timestamp}-{short}"
            stage_root.mkdir(mode=0o700)
            backup_root.mkdir(mode=0o700)
            _fsync_directory(target)

            freeze = _read_json_strict(
                _contained(source, V40_FREEZE_PATH, must_exist=True)
            )
            ...
            try:
                for relative in V40_TRANSITION_FILES:
                    ...
                    if _test_fail_after is not None and len(touched) == _test_fail_after:
                        raise RuntimeError("Injected V4.0 partial-commit fault.")
                ...
                if not (
                    ...
                    and postinstall.get("successor_state") == "V4.0"
                ):
                    raise RuntimeError("Installed overlay failed exact V4.0 authentication.")
...
def _main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Safely install Quantum Lab V4.0.")
