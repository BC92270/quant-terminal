"""Transactional, fail-closed installer for the Quantum Lab V4.8 overlay.

Only an exact authenticated V4.7 predecessor or an exact V4.8 no-op target is
...
INSTALLER_VERSION = "QUANTUM LAB V4.8 TRANSACTIONAL INSTALLER · V1"
LOCK_NAME = ".quantum-lab-v48-install.lock"
RESIDUAL_LOCK_NAMES = (
    ".quantum-lab-v47-install.lock",
    ...
    ".quantum-lab-v41-install.lock",
    ".quantum-lab-v40-install.lock",
    ".quantum-lab-v39-install.lock",
    ".quantum-lab-v38-install.lock",
    ".quantum-lab-v37-install.lock",
    ".quantum-lab-v36-install.lock",
)
PARENT_FREEZE_PATH = "FREEZE_CONTRACT_V4_7.json"
V48_FREEZE_PATH = "FREEZE_CONTRACT_V4_8.json"
README_PATH = "quantum_research_lab/README.md"
UI_PATH = "quantum_research_lab/ui.py"
PARENT_FREEZE_RAW_SHA256 = "89dfc8f614955d4fe1c92cfd286ab04f4022d9eb69e566edf46a65e84aa81a3a"
PARENT_FREEZE_SEMANTIC_SHA256 = "3f870a5ff89368d2000a58533fba395b0d077cc377cdc830fdc4b362f6465858"
PARENT_FROZEN_FILE_COUNT = 272
PARENT_IMMUTABLE_FILE_COUNT = 270
PARENT_FROZEN_PATHS_FINGERPRINT = "ae031bfffbb63989a382fb03aef942481c1a1d685ff0b574217bfb8e4fb037aa"
EXPECTED_V48_FROZEN_FILE_COUNT = 295
EXPECTED_V48_FROZEN_PATHS_FINGERPRINT = "99616a4c8a47f955f124265d4b316e81d8feba4b8ed808dee0d31540a5f446d2"
EXPECTED_V48_OVERLAY_ORDER_SHA256 = "157ea72c59d642eda2bd48f50dd5c413e67d4ab9164638f0cc733022a3e0d843"
...
V48_TRANSITION_FILES = (
    ...
    "install_quantum_lab_v48.py",
    "build_quantum_lab_v48_release.py",
    "outputs/quantum_phase3/v48_multi_snapshot_architecture/SEALED_V4_8_MULTI_SNAPSHOT_ARCHITECTURE_ARTIFACT.json",
    "outputs/quantum_phase3/v48_multi_snapshot_architecture/SEALED_V4_8_VALIDATION_REPORT.json",
    ...
    "quantum_research_lab/QUANTUM_LAB_V4_8_ARCHITECTURE.md",
    "quantum_research_lab/phase3_v48_reference_builder.py",
    "quantum_research_lab/phase3_v48_multi_snapshot_architecture_optimizer.py",
    "quantum_research_lab/phase3_v48_independent_checker.py",
    "quantum_research_lab/phase3_v48_validation.py",
    "quantum_research_lab/phase3_v48_ui.py",
    "quantum_research_lab/test_phase3_v48.py",
    "quantum_research_lab/test_phase3_v48_release.py",
    "quantum_research_lab/verify_freeze_contract_v48.py",
    "quantum_research_lab/verify_phase3_v48.py",
    "quantum_research_lab/verify_phase3_v48_ui.py",
    README_PATH,
    UI_PATH,
)
V48_ONLY_FILES = frozenset(V48_TRANSITION_FILES) - {README_PATH, UI_PATH}
V48_TRANSITION_ORDER_SHA256 = hashlib.sha256(
    json.dumps(list(V48_TRANSITION_FILES), separators=(",", ":")).encode("utf-8")
).hexdigest()
if V48_TRANSITION_ORDER_SHA256 != EXPECTED_V48_OVERLAY_ORDER_SHA256:
    raise RuntimeError("V4.8 ordered overlay inventory drifted.")
...
def source_tree_raw_sha256(source_root: str | Path) -> str:
    ...
    rows = [
        ...
        for relative in V48_TRANSITION_FILES
    ]
    return hashlib.sha256(
...
def _authenticate_source_tree_raw(
    ...
    try:
        ...
        return None, [f"Cannot hash exact V4.8 source tree: {exc}"]
    if actual != expected_source_tree_raw_sha256:
        return actual, [
            "V4.8 source-tree raw identity mismatch: "
...
def _parent_identity(root: Path) -> tuple[dict[str, Any], dict[str, str], list[str]]:
    ...
    try:
        ...
        if not isinstance(frozen, dict):
            raise ValueError("V4.7 frozen_files must be an object")
        if not (
            ...
            and parent.get("freeze_contract_version") == "QUANTUM LAB V4.7 FREEZE CONTRACT · V1"
            ...
            raise ValueError("V4.7 freeze raw, semantic or inventory identity mismatch")
    except Exception as exc:
        return {}, {}, [f"Cannot authenticate exact V4.7 freeze: {exc}"]
    immutable = {
    ...
    if len(immutable) != PARENT_IMMUTABLE_FILE_COUNT:
        errors.append(
            f"V4.7 immutable inventory mismatch: {len(immutable)} != {PARENT_IMMUTABLE_FILE_COUNT}"
        )
...
def _source_inventory(
    ...
    try:
        freeze_path = _contained(source_root, V48_FREEZE_PATH, must_exist=True)
        freeze = _read_json_strict(freeze_path)
    except Exception as exc:
        return {}, [], list(dict.fromkeys(errors + [f"Cannot read source V4.8 freeze: {exc}"]))
    frozen = freeze.get("frozen_files") or {}
    if not isinstance(frozen, dict):
        return freeze, [], list(dict.fromkeys(errors + ["V4.8 frozen_files must be an object."]))
    if not (
        freeze.get("freeze_contract_version") == "QUANTUM LAB V4.8 FREEZE CONTRACT · V1"
        and freeze.get("freeze_contract_sha256") == _freeze_semantic(freeze)
        and freeze.get("frozen_file_count") == len(frozen) == EXPECTED_V48_FROZEN_FILE_COUNT
        and _path_fingerprint(frozen)
        == freeze.get("frozen_paths_fingerprint_sha256")
        == EXPECTED_V48_FROZEN_PATHS_FINGERPRINT
    ):
        errors.append("V4.8 source freeze identity, count or path fingerprint mismatch.")
    deployment = freeze.get("deployment_contract") or {}
    successor = freeze.get("successor_policy") or {}
    if freeze.get("release_paths") != list(V48_TRANSITION_FILES):
        errors.append("V4.8 source release path order mismatch.")
    if not (
        deployment.get("ordered_transition_paths") == list(V48_TRANSITION_FILES)
        and deployment.get("ordered_transition_paths_sha256") == EXPECTED_V48_OVERLAY_ORDER_SHA256
        and deployment.get("overlay_file_count") == len(V48_TRANSITION_FILES)
        and deployment.get("readme_surface_penultimate") == README_PATH
        and deployment.get("integration_surface_last") == UI_PATH
    ):
        errors.append("V4.8 deployment ordering contract mismatch.")
    if not (
        successor.get("accepted_target_states") == ["V4.7", "V4.8"]
        and successor.get("allowed_v47_superseded_files") == [README_PATH, UI_PATH]
        and successor.get("mixed_or_third_state") == "REJECT"
    ):
        errors.append("V4.8 source successor policy mismatch.")
    for relative, expected in parent_immutable.items():
        if frozen.get(relative) != expected:
            errors.append(f"V4.8 freeze altered immutable V4.7 digest: {relative}")
    if frozen.get(PARENT_FREEZE_PATH) != PARENT_FREEZE_RAW_SHA256:
        errors.append("V4.8 freeze does not preserve the exact V4.7 freeze file.")
    ...
    if set(frozen) != expected_paths:
        errors.append("V4.8 frozen path set is not the exact append-only successor inventory.")
    rows: list[dict[str, Any]] = []
    for relative in V48_TRANSITION_FILES:
        try:
            path = _contained(source_root, relative, must_exist=True)
            if relative == V48_FREEZE_PATH:
    ...
    for relative, expected in sorted(frozen.items()):
        try:
            byte_root = (
                source_root
                if relative in V48_TRANSITION_FILES and relative != V48_FREEZE_PATH
                else lineage_root
            )
...
def _successor_state(
    ...
    if pair == (
        parent_policy.get("v47_successor_readme_sha256"),
        parent_policy.get("v47_successor_ui_sha256"),
    ):
        return "V4.7"
    if pair == (
        source_policy.get("v48_successor_readme_sha256"),
        source_policy.get("v48_successor_ui_sha256"),
    ):
        return "V4.8"
    return "INVALID"
    ...
    *,
    ...
    try:
        source_freeze = _read_json_strict(
            _contained(source_root, V48_FREEZE_PATH, must_exist=True)
        )
        if source_freeze.get("freeze_contract_sha256") != _freeze_semantic(source_freeze):
            raise ValueError("V4.8 source freeze self-hash mismatch")
    except Exception as exc:
        source_freeze = {}
    ...
    if len(immutable) != PARENT_IMMUTABLE_FILE_COUNT or mismatches:
        errors.append(f"Target immutable V4.7 mismatch count: {len(mismatches)}")
    try:
    ...
    if state == "V4.7":
        partial = sorted(
            relative
            for relative in V48_ONLY_FILES
            if (target_root / relative).exists() or (target_root / relative).is_symlink()
        )
        if partial:
            errors.append("Partial V4.8 state detected: " + ", ".join(partial))
    elif state == "V4.8":
        ...
        for relative in V48_TRANSITION_FILES:
            try:
                ...
                if actual != expected:
                    errors.append(f"Exact V4.8 target mismatch: {relative}")
            except Exception as exc:
                errors.append(f"Exact V4.8 target path {relative}: {exc}")
    for relative in V48_TRANSITION_FILES:
...
def _transition_root(source_root: Path, predecessor_root: Path | None = None) -> str:
    ...
    if errors or not rows:
        raise ValueError("Cannot commit an unauthenticated V4.8 source: " + "; ".join(errors))
    return hashlib.sha256(
        json.dumps(
...
def _authenticate_candidate_tree(stage: Path, freeze: Mapping[str, Any]) -> None:
    ...
    candidate_freeze = _read_json_strict(_contained(stage, V48_FREEZE_PATH, must_exist=True))
    if candidate_freeze != freeze or candidate_freeze.get("freeze_contract_sha256") != _freeze_semantic(candidate_freeze):
        raise RuntimeError("Candidate V4.8 freeze identity mismatch.")
...
def _build_candidate(
    ...
    stage = Path(tempfile.mkdtemp(prefix="quantum-lab-v48-candidate."))
    try:
        frozen = source_freeze.get("frozen_files") or {}
        ...
        completed = subprocess.run(
            [
                sys.executable,
                "-m",
                "quantum_research_lab.phase3_v48_validation",
                "--root",
                str(stage),
            ],
            cwd=stage,
...
def preflight(
    ...
    *,
    ...
    if not errors and target_report.get("state") == "V4.7":
        try:
    ...
    elif not errors and target_report.get("state") == "V4.8":
        candidate_validation = {
            "performed": False,
            "passed": True,
    ...
    return {
        ...
        "expected_source_tree_raw_sha256": expected_source_tree_raw_sha256,
        "installer": INSTALLER_VERSION,
        "overlay_file_count": len(V48_TRANSITION_FILES),
        "source_lineage_mode": (
...
def _release_lock(lock: Path, token: str) -> None:
    ...
    if not token_path.is_file() or token_path.read_text(encoding="utf-8") != token:
        raise RuntimeError("Refusing to release a V4.8 lock without the owned token.")
    token_path.unlink()
    lock.rmdir()
...
def _inject(requested: str | None, phase: str) -> None:
    if requested == phase:
        raise RuntimeError(f"Injected V4.8 phase fault: {phase}")
...
def apply_overlay(
    ...
    *,
    ...
    if fault_after is not None and not 1 <= fault_after <= len(V48_TRANSITION_FILES):
        return {
            "applied": False,
            "errors": [f"fault_after must be within 1..{len(V48_TRANSITION_FILES)}"],
            "installer": INSTALLER_VERSION,
            "valid": False,
        }
    ...
    if (initial.get("target") or {}).get("state") == "V4.8":
        return {
            "applied": False,
            "errors": [],
    ...
    lock: Path | None = None
    token = ""
    backup = target / ".quantum-lab-v48-backups" / (
    ...
    try:
        ...
        if source_anchor_errors or source_tree_raw_before != initial.get(
            ...
            raise RuntimeError(
                "V4.8 raw source tree changed after preflight: "
        ...
        if source_root_before != initial.get("source_transition_root_sha256"):
            raise RuntimeError("V4.8 source changed after preflight.")
        ...
        if predecessor.get("valid") is not True or predecessor.get("state") != "V4.7":
            raise RuntimeError(
                f"V4.7 predecessor changed before commit: {predecessor}"
            )
        _inject(fault_phase, "after_predecessor_reauthentication")
        backup.mkdir(parents=True, mode=0o700)
        _inject(fault_phase, "after_backup_directory")
        for index, relative in enumerate(V48_TRANSITION_FILES, start=1):
            ...
            destination.parent.mkdir(parents=True, exist_ok=True)
            temporary = destination.with_name(f".{destination.name}.v48-{uuid4().hex}.tmp")
            ...
            if fault_after == index:
                raise RuntimeError(f"Injected V4.8 commit fault after path {index}")
        ...
        if (
            ...
            raise RuntimeError("V4.8 source changed during commit.")
        ...
        if final_target.get("valid") is not True or final_target.get("state") != "V4.8":
